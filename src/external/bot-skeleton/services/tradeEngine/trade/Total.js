...
export default Engine =>
    class Total extends Engine {
        ...
        updateTotals(contract) {
            const { buy_price: buyPrice, currency, profit: contractProfit } = contract;

            const profit = getRoundedNumber(Number(contractProfit), currency);
            ...
            const effectiveSellPrice = win ? Number(contract.payout) : 0;
            accountStat.totalPayout = getRoundedNumber(Number(accountStat.totalPayout) + effectiveSellPrice, currency);
